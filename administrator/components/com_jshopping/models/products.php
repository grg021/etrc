...
class JshoppingModelProducts extends JModel{
    ...
    function _allProductsOrder($order = null, $orderDir = null, $category_id = 0){
        if ($order && $orderDir){
            $fields = array("product_id"=>"pr.product_id", "name"=>"name",'category'=>"namescats","manufacturer"=>"man_name","vendor"=>"v_f_name","ean"=>"ean","qty"=>"qty","price"=>"pr.product_price","hits"=>"pr.hits","date"=>"pr.product_date_added", "product_name_image"=>"pr.product_name_image");
    ...
    function uploadImages($product, $product_id, $post){
        ...
        if (is_array($post['old_image_descr'])){
    ...
    function addToProductImage($product_id, $name_full, $name_image, $name_thumb, $image_descr) {
        ...
        $image->set("image_full", $name_full);
    ...
    function uploadFiles($product, $product_id, $post){
        ...
        for($i=0; $i<$jshopConfig->product_file_upload_count; $i++){
            ...
            if (!$file_sale && $post['product_file_name_'.$i]){
    ...
    function saveAttributes($product, $product_id, $post){
        ...
        if (!is_array($list_saved_attr)) $list_saved_attr = array();
