...
class JshoppingControllerProducts extends JController{
    ...
    function display($cachable = false, $urlparams = false){    
        ...
        $dispatcher->trigger('onBefnoreDisplayListProductsView', array(&$view));
        $view->display();        
    }
    ...
    function save(){
        ...
        if (is_array($post['productfields'])){
    ...
    function _publishProduct($flag) {
        ...
        foreach ($cid as $key => $value) {
            $query = "UPDATE `#__jshopping_products`
                       SET `product_publish` = '" . $db->escape($flag) . "'
                       WHERE `product_id` = '" . $db->escape($value) . "'";
            $db->setQuery($query);
            $db->query();
        }
    ...
    function savegroup(){
        ...
        foreach($cid as $id){
            ...
            if ($post['product_price']!="" || $post['product_old_price']!=""){                
